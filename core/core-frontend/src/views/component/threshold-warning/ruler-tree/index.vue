...
import type { Relation } from "./AuthTree.vue";

const { t } = useI18n();

interface ElementField extends Item {
  logic: string;
  child: FieldItem[];
  items?: FieldItem;
}
...
interface FieldItem extends Item {
  subTree?: ElementField;
  type?: string;
}
const filedList =
  inject<Ref<{ name: string; id: string; deType: number }[]>>("filedList")!;
const errorMessage = ref("");
const logic = ref<"or" | "and">("or");
const relationList = ref<Array<Relation>>([]);
...
const dfsInit = (arr) => {
  const elementList: Array<ElementField | FieldItem> = [];
  arr.forEach((ele) => {
    const { subTree } = ele;
    if (subTree) {
      const { items, logic } = subTree;
      const child = dfsInit(items);
      elementList.push({ logic, child } as ElementField);
    } else {
      const {
        enumValue,
        fieldId,
      ...
      const { name, deType } =
        filedList.value.find((ele) => ele.id === fieldId) || {};
      elementList.push({
        enumValue,
        fieldId,
        valueType,
...
const dfsSubmit = (arr) => {
  const items: Array<Element | FieldItem> = [];
  arr.forEach((ele) => {
    const { child = [] } = ele;
    if (child.length) {
      const { logic } = ele;
...
const calculateDepth = (obj) => {
  ...
  obj.child.forEach((item, index) => {
    ...
    if (item.child?.length) {
      let parent =
        (dfs(obj.child, 0) * 41.4) / 2 + (getY(obj.child) || 0) * 41.4;
