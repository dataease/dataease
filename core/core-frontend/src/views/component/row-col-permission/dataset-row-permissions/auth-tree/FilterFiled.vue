...
import { ref, inject, computed, watch, onBeforeMount, toRefs, Ref } from "vue";
import { useI18n } from "@/hooks/web/useI18n";
import {multFieldValues, multFieldValuesForPermissions} from "@/api/dataset";
import {
  textOptions,
  dateOptions,
  valueOptions,
...
const sysParamsList = inject<Ref>("sys-params-list");
...
const sysParamsIln = computed(() => {
  ...
  if ([0, 5].includes(deType)) {
    return sysParamsList.value.filter((item) => item.type === "text");
  } else if (deType === 1) {
    return sysParamsList.value.filter((item) => item.type === "time");
  } else {
    return sysParamsList.value.filter((item) => item.type === "num");
  }
});
...
const initEnumOptions = () => {
  ...
  if (filterType === "enum" && [0, 5].includes(deType)) {
    multFieldValues({ fieldIds: [fieldId] }).then((res) => {
