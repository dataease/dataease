...
import RoleManage from '@/views/menu/system/member/RoleManage.vue'
import { useI18n } from '@/hooks/web/useI18n'
import GridTable from '@/components/grid-table/src/GridTable.vue'
import { memberPageApi, defaultPwdApi, resetPwdApi, userUnlockApi } from '@/api/user'
        ...
        <el-table-column :label="t('user.role')" min-width="200" show-overflow-tooltip>
          <template #default="{ row }">
            ...
            >
              <template #reference>
                <div class="role-text">
                  <span>{{ (row.roleItems || []).map((r: any) => r.name).join(', ') }}</span>
              ...
              <div class="role-actions">
                ...
                <el-button size="small" type="primary" @click="debounceConfirmRole(row)">{{
                  t('commons.confirm')
                }}</el-button>
              </div>
            </el-popover>
        ...
        <el-table-column width="120" fixed="right" :label="t('common.operate')">
          <template #default="{ row }">
            <div class="operate-icon-container">
              ...
              >
                <div class="reset-pwd-confirm">
                  ...
                  <div class="confirm-foot">
                    <el-button @click="closeResetInfo(row)">{{ t('common.cancel') }}</el-button>
              ...
              <el-tooltip effect="dark" :content="t('setting_mfa.reset_key_tips')" placement="top">
                <el-button text @click="resetMfaHandler(row)">
                  <template #icon>
                    <Icon name="icon_mfa_reset"><icon_mfa_reset class="svg-icon" /></Icon>
                  </template>
                </el-button>
              </el-tooltip>
              <el-tooltip
