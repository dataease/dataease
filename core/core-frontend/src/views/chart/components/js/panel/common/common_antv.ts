...
class CustomZoom extends Zoom {
  resetButtonGroup(container) {
    ...
    const { buttonColor, buttomBackground } = this.controlOption as any
    if (buttonColor) {
      const elements = [
        this.controlOption.zoomInText,
        this.controlOption.zoomOutText,
    ...
    if (buttomBackground) {
      setStyle(elements, 'background', buttomBackground)
    }
...
export function configL7Zoom(chart: Chart, plot: L7Plot<PlotOptions>) {
  ...
  plot.once('loaded', () => {
    const zoomOptions = {
      ...
      buttonColor: basicStyle.zoomBottonColor,
      buttomBackground: basicStyle.zoomBackground
    } as any
