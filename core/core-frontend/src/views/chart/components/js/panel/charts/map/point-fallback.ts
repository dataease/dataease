...
function getPointCoordinates(item: any): number[] | null {
  const coords = item.geometry?.coordinates || item.properties?.center || item.properties?.centroid
 ...
 * 当 GeoJSON 只包含 Point/MultiPoint 时，Choropleth 无法做区域填充，改用 Dot 散点来渲染。
 */
export async function drawPointFallbackChart(
  ...
  rawPoints.forEach(pt => {
    const matchedData = pt.matched?.data || {}
    dotData.push({
      ...matchedData,
      x: pt.x,
      y: pt.y,
  ...
  view.once('loaded', () => {
    ...
    const pointFallbackView = view as any
    pointFallbackView.currentDistrictData = {
      type: 'FeatureCollection',
    ...
    if (!pointFallbackView.source?.data) {
      pointFallbackView.source = { data: { dataArray: dotData.filter(d => d.hasData) } }
    } else {
      pointFallbackView.source.data.dataArray = dotData.filter(d => d.hasData)
    }
    if (!pointFallbackView.tooltip) {
      pointFallbackView.tooltip = {
        destroy: () => undefined,
        options: {
          domStyles: {
            'l7plot-tooltip': {
