<template>
  <div
    id="dataease-v3-embedded-sqlbot"
    v-loading="loading"
    class="dataease-v3-embedded-sqlbot"
  ></div>
</template>
...
const loadSqlbotPage = () => {
  const scriptId = 'dataease-v3-embedded-sqlbot-script'
...
const mountedEmbeddedPage = () => {
  ...
  const tempTimer = setTimeout(() => {
    if (window['sqlbot_embedded_handler']) {
      window['sqlbot_embedded_handler'].mounted('#dataease-v3-embedded-sqlbot', {
        embeddedId: state.id,
        online: true,
