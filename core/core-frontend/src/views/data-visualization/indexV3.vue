...
const canvasCoreStyle = computed(() => {
  return {
    transform: `scale(${canvasStyleData.value.tScale})`
  }
})
  ...
  >
    ...
    >
      ...
      <main id="dv-main-center" class="center" ref="canvasCenterRef">
        ...
        <de-ruler-v3
          ref="deWRulerRef"
          @update:tickSize="val => (state.baseWidth = val)"
        ></de-ruler-v3>
        <de-ruler-v3
          direction="vertical"
        ...
        <ComponentToolBarV3 :class="{ 'preview-aside-x': previewStatus }"></ComponentToolBarV3>
      </main>
      <!-- 右侧侧组件列表 -->
