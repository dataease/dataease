...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate((valid) => {
    if (valid) {
      const param = { ...form };
      const method =
        formType.value === "add" ? variableCreateApi : variableEditApi;
      showLoading();
...
<template>
  <el-dialog
    ...
    :title="
      formType === 'add' ? t('system.add_variable') : t('system.edit_variable')
    "
    width="420px"
    height="282px"
    @open="addKeyDown"
  ...
  >
    ...
    <template #footer>
      <span class="dialog-footer">
        <el-button @click="resetForm(variableForm)">{{
          t("common.cancel")
        }}</el-button>
