...
const drawerMainOpen = async () => {
  ...
  const currentOidVal = Array.isArray(currentOid)
    ? currentOid?.[0]
    : currentOid;
...
const changeSwitch = (row) => {
  ...
  switchEnableApi(param).then(() => {
    ElMessage.success(
      t(row.enable ? "user.enable_success" : "user.disable_success"),
    );
    loading.value = false;
  });
};
...
const treeFilterChangeHandler = ({
  ...
  if (field === "oid") {
    ...
    if (newOid !== oldOidVal || (newOid == null && oldOidVal == null)) {
      ...
      filterOption[3].property.placeholder = newOid
        ? t("commons.role")
        : t("org.select_org_first");
      filterOption[3].property.customPlaceholder = newOid
        ? ""
...
const searchCondition = (conditions) => {
  ...
  if (newOidVal !== oldOidVal) {
    ...
    filterOption[3].property.placeholder = newOidVal
      ? t("commons.role")
      : t("org.select_org_first");
...
<template>
  ...
  >
    ...
    <div
      :class="[
        state.filterTexts.length ? 'is-in-filter' : 'user-table__content',
      ]"
    >
      <GridTable
            ...
            <div class="operate-icon-container" v-if="scope.row.id !== '1'">
              ...
              <div class="user-icon-more">
                <handle-more
                  ...
                  :menu-list="scope.row.id === curUid ? selfMoreList : moreList.filter(item => item.command !== 'unlock' || scope.row.locked)"
                />
              </div>
            </div>
          </template>
