...
const getLogStatusIcon = value => {
  ...
  if (value === 'Success'.toUpperCase() || value === 'unexecuted'.toUpperCase()) {
...
const execute = (row: ITaskInfoRes) => {
  ElMessageBox.confirm(t('sync_task.confirm') + ' ' + t('sync_task.running_one'), {
...
const delHandler = (row: ITaskInfoRes) => {
  ElMessageBox.confirm(t('sync_task.confirm_delete_msg'), {
    ...
    removeApi(row.id).then(() => {
      tableLoading.value = false
      ElMessage.success(t('sync_task.delete') + ' ' + t('sync_task.op_success'))
      search()
    })
  })
}
  ...
  getTaskInfoListApi(
    ...
    state.taskList.forEach(item => {
      ...
      if (resItem) {
        ...
        if (item.currentTime) {
          if (item.currentTime > resItem.executorStartTime || resItem.status === 'running') {
            isRunning = true
          }
          item.currentTime = undefined
        } else if (resItem.status === 'running' || resItem.logStatus === 'RUNNING') {
          isRunning = true
        } else {
...
<template>
  <div class="source-ds-table_task de-search-table">
    <div
    ...
    >
      ...
      <div :class="[state.filterTexts.length ? 'is-in-filter' : 'source-ds-table__content']">
        ...
        >
          <el-table-column type="selection" width="55" :selectable="rowCheckStatus" />
          <el-table-column
            key="name"
          ...
          >
            <template #default="scope">
              <div style="display: flex; align-items: center">
                <el-icon v-if="scope.row.status === 'running'" size="16px" :class="'is-loading'">
                ...
                <el-icon size="16px" v-else>
                  <Icon :style="'color:' + getLogStatusIcon(getLastStatus(scope.row)).color">
                    <component
                      v-if="getLastStatus(scope.row)"
                      :style="'color:' + getLogStatusIcon(getLastStatus(scope.row)).color"
          ...
          >
            <template #default="scope">
              <div class="operate-icon-container" v-if="scope.row.id !== '1'">
                <el-tooltip effect="dark" :content="t('sync_task.edit')" placement="top">
  ...
  ></drawer-main>
  <el-dialog
    v-model="taskIdDialogVisible"
    :show-close="false"
    :title="t('dataset.task_id')"
    width="420"
    align-center
  >
    <template #default>
      <div style="text-align: center">
        ...
        <el-tooltip effect="dark" :content="t('common.copy')" placement="top">
          <el-icon @click="copyInfo(dialogTaskId)" class="hover-icon hover-icon-in-table">
