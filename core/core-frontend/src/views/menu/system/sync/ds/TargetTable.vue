...
const getDsInfo = async data => {
  return getByIdApi(data.id).then(async res => {
    ...
    let { configuration, datasourceRole, desc, id, name, status, statusRemark, type } = res.data
    if (configuration) {
      try {
...
<template>
  <div class="source-ds-table_sync de-search-table">
    <div
    ...
    >
      <el-row class="source-ds-table__filter top-operate">
        <el-col :span="12">
          <el-button @click="createDatasource" type="primary">
            {{
              activeSource ? t('sync_datasource.add_source_ds') : t('sync_datasource.add_target_ds')
            }}
          </el-button>
        </el-col>
      ...
      <div :class="[state.filterTexts.length ? 'is-in-filter' : 'source-ds-table__content']">
        ...
        >
          <el-table-column type="selection" min-width="30" :selectable="rowCheckStatus" />
          <el-table-column
            key="name"
          ...
          <el-table-column key="typeName" prop="typeName" :label="t('sync_datasource.type')" />
          <el-table-column
            key="desc"
            show-overflow-tooltip
            prop="desc"
          ...
          >
            <template #default="scope">
              <div style="display: flex; align-items: center">
                <el-icon size="16px">
                  <Icon :style="scope.row.status === 'Success' ? '' : 'color: red'">
                    <component
                      :is="scope.row.status === 'Success' ? icon_succeed_filled : icon_close_filled"
          ...
          <el-table-column width="110" fixed="right" key="_operation" :label="t('common.operate')">
            <template #default="scope">
              <div class="operate-icon-container">
                ...
                <el-tooltip
                  ...
                  placement="top"
                >
                  <el-button text @click="showSyncFormDialog(scope.row)" class="del-button">
                    <template #icon>
                ...
                <el-tooltip effect="dark" :content="t('common.delete')" placement="top">
                  <el-button text @click="delHandler(scope.row)" class="del-button">
                    <template #icon>
  ...
  <drawer-main
    ...
    ref="drawerMainRef"
  ></drawer-main>
  <EditorDatasource @refresh="refreshGrid" ref="datasourceEditor"></EditorDatasource>
  <SyncForm ref="syncFormDialogRef"></SyncForm>
</template>

<style lang="less">
.source-ds-table_sync {
  line-height: 1.15;
