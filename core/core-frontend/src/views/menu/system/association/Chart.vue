...
`;
          ...
          const icon_data_visualization = svgToDataURI(
            icon_data_visualization_svg
          );
          ...
          const stroke =
            (activeNode && api.value(8)) || !activeNode ? mainColor : "#c0c4cc";
            ...
            children: [
              {
                ...
                style: {
                  text:
                    contextVal!.measureText(val).width > width
                      ? val.slice(0, 17) + "..."
                      : val, // data中取值
                  color: "#1F2329",
                  x: 25,
      ...
      },
      {
        ...
        renderItem: function (params, api) {
          ...
          const stroke =
            (activeNode && api.value(7)) || !activeNode ? mainColor : "#c0c4cc";
          return {
            type: "group", // 当需要多个自定义拼接时，需要用group，此案例是文字和图形的拼接
            children: [
              {
