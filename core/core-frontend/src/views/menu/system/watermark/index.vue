        ...
        <el-col class="main-col-left" :span="12">
          ...
          >
            ...
            >
              <el-select
                :disabled="!state.watermarkForm.enable"
                v-model="state.watermarkForm.type"
              >
                <el-option
                  :label="t('watermark.custom_content')"
                  value="custom"
                />
          ...
          <el-row style="margin-left: 53px; text-align: left">
            <el-button size="middle" @click="cancel"
              >{{ t("watermark.reset") }}
            </el-button>
            <el-button size="middle" @click="preview"
              >{{ t("watermark.preview") }}
            </el-button>
        ...
        <el-col :span="12" style="height: 100%">
          ...
          >
            <div class="demo-preview">
              <img
                style="height: 100%"
                src="@/assets/img/watermark-demo-light.png"
              />
            </div>
            <div class="demo-preview" style="margin-top: 15px">
              <img
                style="height: 100%"
                src="@/assets/img/watermark-demo-dark.png"
              />
            </div>
          </div>
        </el-col>
...
const onValChange = (valKey) => {
  if (
    state.watermarkForm[valKey] === null ||
...
const initWatermark = () => {
  ...
  if (state.watermarkForm.type === "custom") {
    ...
    watermark_txt = watermark_txt.replaceAll(
      "${username}",
      state.userLoginInfo.account,
    );
    watermark_txt = watermark_txt.replaceAll(
      "${nickName}",
      state.userLoginInfo.name,
    );
    watermark_txt = watermark_txt.replaceAll("${time}", getNow());
    watermark_width =
      watermark_txt.length * state.watermarkForm.watermark_fontsize * 0.75;
    ...
    watermark_width =
      watermark_txt.length * state.watermarkForm.watermark_fontsize + 30;
