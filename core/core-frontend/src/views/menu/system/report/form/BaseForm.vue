    ...
    >
      ...
      <el-form-item :label="t('report.form.title')" prop="title">
        <el-input
          ...
          :placeholder="t('common.please_input') + t('report.form.title')"
          @change="titleChange"
        />
      </el-form-item>
      ...
      <el-form-item
        v-if="panelWatermark"
        :label="t('report.form.water_mask')"
        prop="showWatermark"
      >
        <el-checkbox
          v-model="formState.showWatermark"
          :label="t('report.form.show_water_mask')"
        />
      </el-form-item>

      <el-form-item
        :label="t('report.form.data_permission')"
        prop="dataPermission"
      >
        <template v-slot:label>
          <div class="basic-form-info-tips">
            <span class="custom-form-item__label">{{
              t("report.form.data_permission")
            }}</span>
            <el-tooltip
              effect="dark"
        ...
        <el-radio-group
          v-model="formState.dataPermission"
          @change="dataPermissionChange"
        >
          <el-radio :label="0">{{
            t("report.form.creator_permission")
          }}</el-radio>
      ...
      >
        ...
        <el-select
          ...
          :max-collapse-tags="3"
          :placeholder="t('common.please_select') + t('report.form.view_data')"
          style="width: 100%"
      ...
      <el-form-item :label="t('report.form.pixel')" prop="pixel">
        <el-select
          ...
          clearable
          allow-create
          :placeholder="t('common.please_select') + t('report.form.pixel')"
      ...
      <el-form-item :label="t('report.form.ext_wait_time')" prop="extWaitTime">
        <el-input-number
          ...
          :max="600"
          :placeholder="
            t('common.please_input') + t('report.form.ext_wait_time')
          "
      ...
      <el-form-item :label="t('report.form.render_time')" prop="renderTime">
        <el-input-number
          ...
          :max="30"
          :placeholder="
            t('common.please_input') + t('report.form.render_time')
          "
          controls-position="right"
          type="number"
        />
...
const setHeight = () => {
  setTimeout(() => {
    const input = viewSelect.value.$el.querySelector(
      "input",
    ) as HTMLInputElement;
...
const loadRTree = () => {
  ...
  return queryTreeApi(request).then((rsp) => {
    ...
    if (!validateRid()) {
      ElMessage.error(
        `当前${busiFlag}[${formState.value.rid}]失效，已自动清空！`,
      );
      formState.value.rid = null;
    }
    loadViewOption();
