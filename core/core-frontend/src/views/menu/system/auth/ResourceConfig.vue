  ...
  const buildCallback = (type: number, item, originItem) => {
    if (type === 0) {
      ...
      whileLoop(cols, true, col => {
        ...
        temp['show'] = temp['show'] || (!levelMatch(originLevelobj, weightLevel) && roleMatch)
        if (roleMatch) {
          const extendKey = originItem && originItem['type'] === 2 ? 'orgs' : 'roles'
...
const cascadeCheckAll = item => {
  ...
  if (check) {
    ...
    whileLoop(state.tableColumn, true, col => {
      ...
      if (curLevel >= level && (!standalone || curLevel === 9 || curLevel === level)) {
        col['checkAll'] = false
      }
    })
  }
  ...
  if (isMenu) {
    while (stack.length) {
      ...
      if (
        ...
        !row['hidden'] &&
        !row.root &&
        !(activeName.value === 'role' && isRoleCheckboxHidden(row, level, 'menu'))
      ) {
        row['value' + level] = check
        rowWeightChanged(row, level)
      }
...
const rowWeightChanged = (row, level) => {
  ...
  if (row.type === 2) {
    ...
    if (row.children?.length) {
      for (const child of row.children) {
        ...
        if (activeName.value === 'role' && isRoleCheckboxHidden(child, level, activeAuth.value))
          continue
        // 根角色隐式权限不可编辑，跳过
        if (activeName.value === 'role' && child.attrs?.root && child['level' + level]?.show)
          continue
  ...
  const isRootResource = activeAuth.value === 'resource' && selectedResourceId.value === '0'
  const check = row['value' + level]
  const checkStandalone = standaloneNode(level)
  const realLevel = getExtLevel(level)
  const standalone = realLevel === 4
...
const loadDsPlugins = () => {
  request
    .get({ url: '/xpackComponent/dsPlugins' })
    .then(res => {
      const map = {}
...
<template>
    ...
    <el-scrollbar v-if="activeAuth === 'menu'" class="menu-tree">
      ...
      >
        <template #default="{ node, data }">
          <span class="custom-tree-node f18" :class="{ 'is-disabled': !data.leaf }">
  ...
  <div v-if="activeAuth === 'resource'" class="resource-list">
    ...
    <el-scrollbar class="resource-tree">
      ...
      >
        <template #default="{ node, data }">
          <span class="custom-tree-node f18" :class="{ 'is-disabled': node.disabled || data.root }">
            <el-icon v-if="!data.leaf">
              <Icon name="dv-folder"><dvFolder class="svg-icon" /></Icon>
            </el-icon>
            ...
            <el-icon v-else-if="selectedResourceType === 'screen'">
              <Icon name="dv-screen-spine"><dvScreenSpine class="svg-icon" /></Icon>
            </el-icon>
            ...
            <span
              class="label-tooltip"
              :title="data.name"
              v-html="data.colorName && resourcekey ? data.colorName : node.label"
            />
          </span>
        </template>
      </el-tree>
    ...
    <template v-else>
      <div class="tab-search">
        ...
        <div class="search-table-bt">
          <el-button v-if="!state.uncommitted.length" disabled @click="save" type="info">{{
            t('common.save')
          }}</el-button>
          <el-button v-else @click="save" type="primary">{{ t('common.save') }}</el-button>
        </div>
      </div>
      <div class="resource-table">
        <div class="tree-table">
          ...
          >
            <template #empty>
              <empty-background :description="t('data_set.no_data')" img-type="tree" />
            </template>
            <vxe-column
              field="name"
            ...
            >
              <template #default="scope">
                <div v-if="scope.row.type === 2" class="de-row-disabled">
                  <span
                    v-html="scope.row.colorName && targetkey ? scope.row.colorName : scope.row.name"
                  />
                </div>
                <div v-else-if="activeName === 'role'" class="list-item_primary-row">
                  <span class="flex-align-center label">
                    <span
                      v-html="
                        scope.row.colorName && targetkey ? scope.row.colorName : scope.row.name
                      "
                    />
                    <span v-if="scope.row.attrs?.root" class="mark flex-center">{{
                      t('role.system')
                    }}</span>
                ...
                >
                  <span
                    v-html="scope.row.colorName && targetkey ? scope.row.colorName : scope.row.name"
                  />
                  <span class="user-account">{{ ` (${scope.row.attrs?.account || ''})` }}</span>
                </span>
              </template>
            </vxe-column>
                  ...
                  <template #header>
                    <div class="dynamic-table-header">
                      <el-tooltip
                        ...
                        placement="top"
                      >
                        <el-checkbox v-model="kidItem['checkAll']" @change="onCheckAll(kidItem)" />
                      </el-tooltip>
                ...
                <template #header>
                  <div class="dynamic-table-header">
                    <el-tooltip
                      ...
                      placement="top"
                    >
                      <el-checkbox v-model="item['checkAll']" @change="onCheckAll(item)" />
                    </el-tooltip>
                    <span>{{ item.label }}</span>
                  </div>
                </template>
