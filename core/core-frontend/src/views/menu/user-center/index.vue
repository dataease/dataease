...
import WecomQr from "../../component/login/WecomQr.vue";
import DingtalkQr from "../../component/login/DingtalkQr.vue";
import LarkQr from "../../component/login/LarkQr.vue";
import LarksuiteQr from "../../component/login/LarksuiteQr.vue";
import { useUserStoreWithOut } from '@/store/modules/user'
import {
  unBindApi,
  bindStatusApi,
  queryCategoryStatus,
} from "../../component/login/bind";
...
const queryForm = () => {
  personInfoApi().then((res) => {
    ...
    const roleNames = data.roleIds?.length ? data.roleIds.map((roleId) => map[roleId]?.name).filter((item) => !!item) : [];
...
const bindHandler = (index) => {
  ...
  loginTip.value =
    t("userCenter.pls_use") + `${item.name}` + t("userCenter.bind_use_qr");
              ...
              <el-col :span="12">
                <div class="base-info-item">
                  ...
                  <p class="value" v-if="staticForm.roleNames?.length">
                    <span
                      v-for="(roleName, index) in staticForm.roleNames"
                      :key="roleName"
                      class="role"
                      >{{ roleName }}</span
                    >
            ...
            <el-row>
              <el-col :span="12">
                <div class="base-info-item">
                  ...
                  <p class="value">
                    {{
                      staticForm.mfaEnable
                        ? t("commons.enable")
                        : t("setting_mfa.status_0")
                    }}
                  </p>
                </div>
              </el-col>
  ...
  >
    <div class="qr-code-img">
      <div class="bind-qr-container" :class="`bind-qr-${optIndex}`">
        ...
        <dingtalk-qr
          v-if="bindDialogVisible && optIndex === 1"
          :is-bind="true"
        />
        <lark-qr v-if="bindDialogVisible && optIndex === 2" :is-bind="true" />
        <larksuite-qr
          v-if="bindDialogVisible && optIndex === 3"
          :is-bind="true"
        />
      </div>
    </div>
