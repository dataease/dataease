...
const switchApiEnable = (row) => {
  ...
  request.post({ url, data }).then(() => {
    ElMessage.success(row.enable ? t("userCenter.enable_success") : t("userCenter.disabled_success"));
  });
};
...
<template>
  <div class="api-key" v-loading="bindLoading">
    <div class="editer-form-title">
      ...
      <span class="pwd"
        >  {{ t("userCenter.api_key_desc") }} </span
      >
    </div>
      ...
      >
        ...
        <el-table-column width="220" label="Secret Key">
          <template #default="scope">
            ...
            <el-tooltip
              effect="dark"
              :content="scope.row.show ? t('userCenter.click_to_hind') : t('userCenter.click_to_show')"
              placement="top"
            >
              <el-icon
                @click="switchShow(scope.row)"
                class="hover-icon hover-icon-in-table"
              >
                <Icon
                  ><component
