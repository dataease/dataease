...
import SpreadsheetDetailInfo from "./components/SpreadsheetDetailInfo.vue";
import SpreadsheetFolderDialog from "./components/SpreadsheetFolderDialog.vue";
import {
...
import { treeWithAuth } from "./api/auth";
...
const isSpreadsheetUnpublished = (data: SpreadsheetTreeNode) =>
  !!data.leaf &&
  getSpreadsheetStatus(data) === SpreadsheetPublishStatus.Unpublished;
  ...
  return treeWithAuth(request)
    .then((res) => {
      ...
      if (
        ...
        originResourceTree.value = JSON.parse(
          JSON.stringify(unref(state.spreadsheetTree)),
        );
...
const defaultProps = {
  ...
  disabled: (data: SpreadsheetTreeNode) =>
    !data.weight || isSpreadsheetUnpublished(data),
};
...
const spreadsheetFolderDialogRef = ref<
  InstanceType<typeof SpreadsheetFolderDialog>
>();
const nickName = ref("");
  ...
  requestId = setTimeout(async () => {
    try {
      ...
      const parsedWorkbookData =
        parseSheetData(data.sheetData) ?? createDefaultWorkbookData();
...
const handleFullscreenChange = () => {
  ...
  const fullscreenPreview =
    document.fullscreenElement === previewShellRef.value;
    ...
    try {
      ...
      if (selectedNodeInfo.id === data.id) {
        selectedNodeInfo.status =
          result.status ?? SpreadsheetPublishStatus.Unpublished;
      }
  ...
  >
          ...
          <div class="icon-methods">
            ...
            <div v-if="rootManage" class="flex-align-center">
              ...
              <el-tooltip
                ...
                offset="14"
                arrow-offset="10"
                :content="t('spreadsheet.new_sheet')"
                placement="top"
              >
                <el-icon
                  class="custom-icon btn"
          ...
          >
            <template #default="{ node, data }">
              ...
              >
                ...
                <el-icon v-if="data.leaf" style="font-size: 18px">
                  <Icon name="icon_spreadsheet"
                    ><icon_spreadsheet class="svg-icon"
                  /></Icon>
                </el-icon>
                <el-tooltip
                  v-if="!data.weight"
                ...
                <div class="icon-more" v-if="data.weight >= 7">
                  ...
                  <handle-more
                    @handle-command="
                      (cmd) =>
                        operation(cmd, data, data.leaf ? 'sheet' : 'folder')
                    "
    ...
    >
      <template v-if="!state.spreadsheetTree.length && mounted">
        <empty-background
          :description="t('spreadsheet.no_data')"
          img-type="none"
        >
          <el-button
            v-if="rootManage"
      ...
      <template v-else-if="!!selectedNodeInfo.id">
          ...
          <div class="info-method">
            <div class="info-left">
              <div class="spreadsheet-title">
                <span
                  :title="selectedNodeInfo.name"
                  class="spreadsheet-name ellipsis"
                  >{{ selectedNodeInfo.name }}</span
                >
                <span
                  v-if="
                    selectedNodeInfo.status ===
                    SpreadsheetPublishStatus.SavedUnpublished
                  "
        ...
        <div
          ref="previewShellRef"
          class="spreadsheet-preview-area"
          v-loading="previewLoading"
        >
          <div
            v-if="isPreviewFocus && !isFullscreenPreview"
            class="preview-mode-actions"
          >
            <el-button @click="handleFullscreen">
              {{ t("visualization.fullscreen_preview") }}
            </el-button>
    ...
    <SpreadsheetFolderDialog
      ref="spreadsheetFolderDialogRef"
      @success="getSpreadsheetTree"
    />
