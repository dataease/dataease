      ...
      <template v-else-if="!isRange">
        <div class="time-config__nested-row">
          <el-radio-group v-model="condition.timeDynamicDefault.offset.relativeToCurrent">
            <el-radio v-for="item in relativeOptions" :key="item.value" :label="item.value">{{ item.label }}</el-radio>
          </el-radio-group>
        </div>
        <el-form-item v-if="condition.timeDynamicDefault.offset.relativeToCurrent === 'custom'" label="时间">
          <div
