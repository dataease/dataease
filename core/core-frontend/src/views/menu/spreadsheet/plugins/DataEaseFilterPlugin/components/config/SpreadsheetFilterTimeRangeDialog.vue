...
const singlePickerTypes = {
  year: 'year', month: 'month', date: 'date', datetime: 'datetime'
} as const
const rangePickerTypes = {
  year: 'yearrange', month: 'monthrange', date: 'daterange', datetime: 'datetimerange'
} as const
...
const rangeRelativeOptions = computed(() => {
  ...
  if (granularity === 'month') return [
    { label: '本月', value: 'thisMonth' }, { label: '上月', value: 'lastMonth' },
    { label: '本季度', value: 'thisQuarter' }, { label: '最近3个月', value: 'LastThreeMonths' },
    { label: '最近6个月', value: 'LastSixMonths' }, { label: '最近12个月', value: 'LastTwelveMonths' },
    { label: '年初至本月', value: 'YearToThisMonth' }, { label: '年初至上月末', value: 'YearToLastMonthEnd' },
    { label: '自定义', value: 'custom' }
  ]
  return [
    ...
    { label: '月初至昨天', value: 'monthToYesterday' }, { label: '自定义', value: 'custom' }
  ]
})
...
const ensure = () => {
  ...
  local.start.dynamic ||= { value: 0, unit: 'day', direction: 'before', relativeToCurrent: 'custom' }
...
<template>
  <div class="time-range-popover">
    ...
    <template v-if="local.intervalType !== 'none'">
      <div class="time-range-popover__label">{{ local.intervalType === 'timeInterval' ? '时间区间' : '时间' }}</div>
      ...
      <template v-if="activeBoundary.type === 'dynamic'">
        ...
        <template v-if="activeRelativeToCurrent === 'custom'">
          <div v-for="side in activeSides" :key="side" class="time-range-popover__form-row">
            <span>{{ local.intervalType === 'timeInterval' ? (side === 'start' ? '开始时间' : '结束时间') : '时间' }}</span>
            <div
              class="time-range-popover__custom"
              :class="{ 'time-range-popover__custom--with-time': baseGranularity === 'datetime' }"
            >
              <el-input-number v-model="local[side]!.dynamic!.value" :min="0" controls-position="right" />
              <el-select v-model="local[side]!.dynamic!.unit" :teleported="false"><el-option v-for="unit in units" :key="unit.value" v-bind="unit" /></el-select>
              <el-select v-model="local[side]!.dynamic!.direction" :teleported="false"><el-option label="前" value="before" /><el-option label="后" value="after" /></el-select>
              <el-time-picker
      ...
      <div v-else class="time-range-popover__form-row">
        ...
        <div class="time-range-popover__picker-control">
          ...
          />
          <el-date-picker v-else v-model="activeBoundary.value" style="width: 100%" :type="pickerType" :value-format="pickerValueFormat" :teleported="false" />
        </div>
      </div>
      <div class="time-range-popover__form-row">
        <span>预览</span>
        <div class="time-range-popover__picker-control">
          <el-date-picker v-if="local.intervalType === 'timeInterval'" :model-value="preview" style="width: 100%" :type="pickerType" disabled />
          <el-date-picker v-else :model-value="preview" style="width: 100%" :type="pickerType" disabled />
        </div>
      </div>
    </template>
  </div>
...
<style scoped>
.time-range-popover { padding: 8px 4px 16px; }
.time-range-popover h4 { margin: 0 0 20px; font-size: 16px; }
.time-range-popover__label { margin: 16px 0 10px; }
.time-range-popover__radios { display: flex; flex-wrap: nowrap; gap: 14px; }
.time-range-popover__form-row { display: grid; grid-template-columns: 96px minmax(0, 1fr); align-items: center; gap: 12px; width: 100%; min-width: 0; margin-top: 12px; }
.time-range-popover__form-row > :last-child { min-width: 0; }
.time-range-popover__custom { display: grid; grid-template-columns: minmax(0, 1fr) 72px 72px; gap: 8px; min-width: 0; }
.time-range-popover__custom--with-time { grid-template-columns: minmax(0, 1fr) 64px 64px 104px; }
.time-range-popover__custom > * { width: 100%; min-width: 0; }
.time-range-popover__picker-control { width: 100%; min-width: 0; }
.time-range-popover__picker-control :deep(.el-date-editor) { width: 100% !important; max-width: 100%; min-width: 0; box-sizing: border-box; }
.time-range-popover__form-row :deep(.el-input-number) { width: 100%; min-width: 0; }
</style>
