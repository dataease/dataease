...
export class PivotTableEditProtectionService {
  ...
  assertCommandAllowed(commandInfo: Readonly<ICommandInfo>): void {
    ...
    if (commandInfo.id === SetCellEditVisibleOperation.id) {
      throw new CustomCommandExecutionError('Pivot table render range is readonly')
    }

    ElMessage.warning('透视表渲染区域不允许编辑')
    throw new CustomCommandExecutionError('Pivot table render range is readonly')
  }
  ...
  private shouldBlock(commandInfo: Readonly<ICommandInfo>): boolean {
    ...
    return ranges.some(range =>
      this.displayStateService.list().some(state =>
        state.sheetId === range.sheetId && this.isRangeAffected(commandInfo.id, range, this.toProtectedRange(state))
      )
    )
  }
  ...
  private isRangeAffected(commandId: string, range: ProtectedRange, protectedRange: ProtectedRange): boolean {
    ...
    return this.rowsOverlap(range, protectedRange) && this.columnsOverlap(range, protectedRange)
  }

  private toProtectedRange(state: PivotTableDisplayState): ProtectedRange {
