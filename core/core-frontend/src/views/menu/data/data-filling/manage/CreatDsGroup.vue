...
import {listDataFillingForms, save} from '../data-filling'
...
const createInit = (type, data: Tree, exec, name: string) => {
  ...
  nodeType.value = type
  filterText.value = ''
  placeholder.value = type === 'folder' ? t('data_set.a_folder_name') : t('data_fill.p_data_fill_name')
  dsType = data.type
  if (type === 'data-filling') {
    request = data.request
  }
  if (exec !== 'rename') {
    listDataFillingForms({ leaf: false, weight: 7 }).then(res => {
...
const saveDataset = () => {
  dataFilling.value.validate(result => {
    if (result) {
      const params: Omit<DataFillingOrFolder, 'nodeType'> & { nodeType: 'folder' | 'data-filling' } = {
      ...
      if (request) {
        ...
        method({...request, name: datasetForm.name, pid: params.pid, nodeType: 'form'})
              .then(res => {
                if (res !== undefined) {
                  useEmitt().emitter.emit('onDfSave', res)
                  ElMessage.success(t('data_fill.save_df_success'))
                  successCb()
                }
              })
              .finally(() => {
  ...
  >
    ...
    >
      <el-form-item v-if="showName" :label="labelName" prop="name">
        <el-input :placeholder="placeholder" v-model="datasetForm.name" @blur="trimName(datasetForm)"/>
      </el-form-item>
      ...
      <div v-if="cmd === 'move'">
        <el-input style="margin-bottom: 12px" v-model="filterText" clearable>
          <template #prefix>
            <el-icon>
              <Icon name="icon_search-outline_outlined"><icon_searchOutline_outlined class="svg-icon" /></Icon>
            </el-icon>
          </template>
        </el-input>
