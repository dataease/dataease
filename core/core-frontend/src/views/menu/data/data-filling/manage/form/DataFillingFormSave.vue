    ...
    _list = map(
      ...
      (e) => {
        return {
          ...e,
          disabled:
            e.type === "YEAR" || e.type === "TIME" || e.type === "TIMETZ",
        };
      },
    );
  }
...
onMounted(() => {
  ...
  Promise.all([p2])
    .then((val) => {
      ...
      if (
        formData.value.pid &&
        !findNodeInTree(folder.value, formData.value.pid)
      ) {
        formData.value.pid = "";
      }
    })
    ...
    <el-main class="de-main">
      ...
      >
        ...
        >
          ...
          >
            <el-option-group
              v-for="(x, $index) in _datasourceList"
              :key="$index"
              :label="x.name"
            >
              <el-option
        ...
        >
          ...
          <el-radio-group
            v-model="formData.useExistsTable"
            @change="onChangeUseExistsTable"
          >
            <el-radio :label="false">{{
              t("data_fill.form.create_new_table")
            }}</el-radio>
            <el-radio :label="true">{{
              t("data_fill.form.bind_exists_table")
            }}</el-radio>
          </el-radio-group>
        </el-form-item>
        ...
        >
          ...
          <template v-if="!formData.useExistsTable">
            <el-input
              v-model.trim="formData.tableName"
              required
              maxlength="50"
              show-word-limit
            />
            ...
            <template #default="scope">
              <el-row :gutter="8">
                <el-col
                  :span="8"
                  v-if="
                    (!isEdit && formData.useExistsTable) ||
                ...
                >
                  <el-form-item class="form-item no-margin-bottom">
                    ...
                    >
                      <el-option
                        :label="t('data_fill.form.create_new_column')"
                        :value="false"
                      />
                      <el-option
                        :label="t('data_fill.form.select_exists_column')"
                        :value="true"
                      />
                    </el-select>
                  </el-form-item>
                ...
                >
                  <el-form-item
                    ...
                    :prop="
                      'forms[' + scope.$index + '].settings.mapping.columnName'
                    "
            ...
            <template #default="scope">
              ...
              >
                ...
                >
                  ...
                  >
                    <el-option
                      v-for="(x, $index) in isEdit && scope.row.old
                        ? allColumnsList
                        : columnsList"
                      :key="$index"
                      :value="x.value"
              ...
              <el-button
                v-if="
                  scope.row.columns.length < 5 && !(isEdit && scope.row.old)
                "
                type="text"
