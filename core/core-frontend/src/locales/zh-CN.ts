export default {
  ...
  role: {
    ...
    delete_tips:
      '<div id="u7755_text" class="text" style="font-size: 12px;"><p><span style="color:#F59A23;">友情提示，角色被删除后，将用户从该角色中移除</span></p></div>',
  ...
  datasource: {
    ...
    ck_ssl_upload: '上传证书',
    ck_ssl_upload_hint: '支持 .pem/.crt/.cer/.key 格式，上传后自动填入内容',
    ck_ssl_pem_placeholder: '输入或上传 PEM 内容',
    ck_ssl_read_failed: '读取 SSL 证书文件失败',
  ...
  chart: {
    ...
    chart_box_plot: '箱线图',
    box_plot_low: '下须（非异常最小值）',
    box_plot_q1: '下四分位数（Q1）',
    box_plot_median: '中位数',
    box_plot_quartile_range: '四分位区间（Q1–Q3）',
    box_plot_q3: '上四分位数（Q3）',
    box_plot_high: '上须（非异常最大值）',
    box_plot_outlier: '异常值',
    box_plot_samples: '{count} 个样本',
    box_plot_more_outliers: '另有 {count} 个',
    ...
    box_plot_outlier_color: '异常点颜色',
    box_plot_outlier_follow_series: '跟随系列',
    box_plot_outlier_custom_color: '自定义颜色',
    box_plot_outlier_size: '异常点大小',
    box_plot_category_tip: '每个类别生成一个箱体，用于比较各类别的数据分布',
    box_plot_group_tip: '可选；在同一类别下拆分多个箱体进行对比',
    box_plot_value_tip: '使用每条有效明细值计算四分位数，不进行求和或平均聚合',
    ...
    custom_tooltip_content_tip:
      "可以${'{'}fieldName{'}'}的形式读取字段值（支持部分安全HTML标签，不支持属性和样式）",
    ...
    total_custom_background: '自定义背景色',
  ...
  online_map: {
    ...
    min_zoom: '最小层级',
    max_zoom: '最大层级',
    zoom_tip: '缩放范围为 0-24，最多保留 1 位小数；不会增加服务本身不存在的地图细节',
  ...
  setting_basic: {
    ...
    spreadsheetQueryLimit: '电子表格查询数量上限',
  ...
  sync_datasource: {
    ...
    auth_method: '认证方式',
    passwd: '用户名密码',
    kerbers_info: '请确保 krb5.Conf、Keytab Key，已经添加到路径：/opt/3.0/conf',
    client_principal: 'Client Principal',
    keytab_Key_path: 'Keytab Key Path',
    data_base: '数据库名称',
    user_name: '用户名',
  ...
  sync_task: {
    ...
    source_field: '源字段',
    source_field_type: '源类型',
    field_mapping_blocking_title: '当前有 {0} 个字段需要处理后才能保存',
    field_mapping_unknown_detail: '目标类型尚未确认，请选择目标数据库支持的字段类型',
    field_mapping_blocking_save_tip: '还有 {0} 个字段的目标类型未确认：{1}，请处理后再保存',
    field_mapping_warning_title: '当前有 {0} 个字段存在同步风险（确认后可继续保存）',
    field_mapping_warning_item: '{0}：{1}',
    field_name_separator: '、',
    field_name_more_suffix: '，另有 {0} 个',
    field_mapping_warning_confirm_title: '存在同步风险',
    field_mapping_warning_confirm_message:
      '当前有 {0} 个字段存在非阻断同步风险，详细信息已显示在字段映射上方。是否仍然保存？',
    field_mapping_save_anyway: '仍然保存',
    field_mapping_back_to_check: '返回检查',
    field_mapping_expand_all: '展开全部（{0} 个字段）',
    field_mapping_collapse: '收起',
    field_type: '类型',
    field_length: '长度',
    field_precision: '精度',
    field_key: '键',
    key_required_by_target: '当前目标数据库和同步模式必须至少配置一个 Key 字段',
    key_policy_load_failed: '获取目标数据库 Key 策略失败',
    field_index: '索引',
    field_comment: '注释',
  ...
  spreadsheet: {
    ...
    save: '保存',
    save_success: '保存成功',
    draft_close_title: '关闭配置',
    draft_close_message: '当前实例还未获取数据，关闭后会清除实例，是否确认？',
    draft_close_confirm: '确认关闭',
    ...
    publish_error: '发布失败',
    publish_status: '发布状态',
    recover_publish: '恢复到发布版本',
    recover_publish_confirm: '恢复后，未发布的修改将被覆盖。确定恢复到发布版本？',
    recover_publish_success: '已恢复到发布版本',
    recover_publish_error: '恢复发布版本失败',
    cancel_publish_confirm: '取消发布后，该电子表格不能被正式查看。确定取消发布？',
    cancel_publish_success: '取消发布成功',
    cancel_publish_error: '取消发布失败',
    saved_unpublished: '有未发布修改',
    preview: '预览',
    export: '导出',
    share: '分享',
    copy: '复制',
    ...
    discard: '不保存',
    load_error: '加载失败',
    load_failed: '电子表格加载失败',
    retry: '重试',
    render_failed: '渲染失败',
    data_empty: '数据为空',
    unknown_load_error: '未知加载错误',
    preview_load_failed: '电子表格“{name}”加载失败：{reason}',
    save_error: '保存失败',
    update_success: '更新成功',
    update_error: '更新失败',
    ...
    dataset_replacement: {
      ...
      more: '更多',
      component_title: '替换组件数据集',
      current_component_dataset: '当前组件数据集',
      replace_to_dataset: '替换到数据集',
      used_fields: '已使用字段',
      matched_fields: '已匹配字段',
      select_dataset_first: '请先选择数据集',
      incomplete_warning: '还未替换数据集和字段',
      confirm: '确定',
      tip: '替换后，电子表格中使用该数据集的组件和字段将同步更新。',
      used_dataset: '已使用数据集',
      used_dataset_count: '已使用数据集：{count}',
      filter_component: '过滤组件',
      target_dataset: '目标数据集',
      select_dataset: '请选择数据集',
      dataset: '数据集',
      refresh: '刷新',
      no_related_data: '暂无相关数据',
      create_dataset: '新建数据集',
      original_field: '原字段',
      target_field: '目标字段',
      field_count: '字段：{count}',
      select_field: '请选择字段',
      search: '搜索',
      search_field: '搜索字段',
      clear_matches_confirm: '确定清空所有已匹配字段吗？',
      no_search_result: '暂无搜索结果',
      no_used_dataset: '当前电子表格未使用数据集',
      usage_summary: '已被 {components} 个组件使用，共 {fields} 个字段',
      unmatched_count: '还有 {count} 个字段未匹配',
      all_matched: '字段已全部匹配',
      replace: '替换',
      success: '数据集替换成功',
      failed: '数据集替换失败',
      refresh_warning: '数据集已替换，{count} 个组件刷新失败，请稍后手动刷新'
    },
    tips: '提示',
