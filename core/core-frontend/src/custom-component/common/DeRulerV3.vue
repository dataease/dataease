    ...
    return {
      left:
        (props.direction === 'horizontal'
          ...
          : curComponent.value.style.top + parentStyle.value.top) *
          canvasStyleData.value.tScale +
        'px',
      width:
        (props.direction === 'horizontal'
...
const ticks = computed(() => {
  ...
  while (currentValue <= rulerSize.value) {
    ...
    result.push({ position: currentValue * canvasStyleData.value.tScale, label, isLong })
...
const tickSize = computed(
  () =>
    10 *
    Math.max(Math.floor((2000 * radio.value) / (rulerSize.value * canvasStyleData.value.tScale)), 1)
)

const scaleWidth = computed(() => rulerSize.value * canvasStyleData.value.tScale)
...
const tickSizeScale = computed(() => tickSize.value * canvasStyleData.value.tScale)
