...
import {compareItem} from '@/utils/map'
...
export default {
  ...
  methods: {
    checkViewTitle(opt, id, tile) {
      try {
        const curPanelViewsData = this.$store.state.componentViewsData
        const curComponentViewNames = []
        this.$store.state.componentData.forEach(item => {
    ...
    getChartDetails(id, panelId, data) {
    ...
    changeEditStatus() {
      ...
      if (this.checkViewTitle('update', this.chart.id, this.chartTitleUpdate)) {
    ...
    getChartDetail() {
      this.getChartDetails(this.chart.id, this.panelInfo.id, {
        ...
        if (!viewSave) return
        this.viewEditSave(this.panelInfo.id, viewSave).then(() => {
          this.chart.title = this.chartTitleUpdate
        })
        this.bus.$emit('title-name', this.chart.title, chartView.id)
        this.bus.$emit('view-in-cache', {
          type: 'styleChange',
          viewId: chartView.id,
          viewInfo: view
        })
