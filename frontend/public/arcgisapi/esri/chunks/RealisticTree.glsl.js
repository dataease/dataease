...
function(e,c,l,m,f,n,d,p,q,r,t,u,v,w,x,y,z,A,B,C,D,E,g){function h(a){const b=new m.ShaderBuilder,F=b.vertex.code,k=b.fragment.code;b.vertex.uniforms.add("proj","mat4").add("view","mat4").add("camPos","vec3").add("localOrigin","vec3");b.include(A.PositionAttribute);b.varyings.add("vpos","vec3");b.include(n.VisualVariables,a);b.include(v.InstancedDoublePrecision,a);b.include(p.VerticalOffset,a);if(0===a.output||7===a.output)b.include(z.NormalAttribute,a),b.include(l.Transform,{linearDepth:!1}),a.offsetBackfaces&&
b.include(y.Offset),a.instancedColor&&b.attributes.add("instanceColor","vec4"),b.varyings.add("vNormalWorld","vec3"),b.varyings.add("localvpos","vec3"),b.include(w.TextureCoordinateAttribute,a),b.include(t.ForwardLinearDepth,a),b.include(B.SymbolColor,a),b.include(q.VertexColor,a),b.vertex.uniforms.add("externalColor","vec4"),b.varyings.add("vcolorExt","vec4"),F.add(c.glsl`
        void main(void) {
      ...
      `);7===a.output&&(b.include(f.Slice,a),b.include(d.DiscardOrAdjustAlpha,a),b.fragment.uniforms.add("camPos","vec3").add("localOrigin","vec3").add("opacity","float").add("layerOpacity","float"),b.fragment.uniforms.add("view","mat4"),a.hasColorTexture&&b.fragment.uniforms.add("tex","sampler2D"),b.fragment.include(g.MixExternalColor),k.add(c.glsl`
      void main() {
        discardBySlice(vpos);
    ...
    `));0===a.output&&(b.include(f.Slice,a),b.include(E.EvaluateSceneLighting,a),b.include(D.EvaluateAmbientOcclusion,a),b.include(d.DiscardOrAdjustAlpha,a),a.receiveShadows&&b.include(r.ReadShadowMap,a),b.fragment.uniforms.add("camPos","vec3").add("localOrigin","vec3").add("ambient","vec3").add("diffuse","vec3").add("opacity","float").add("layerOpacity","float"),b.fragment.uniforms.add("view","mat4"),a.hasColorTexture&&b.fragment.uniforms.add("tex","sampler2D"),b.include(x.PhysicallyBasedRenderingParameters,
a),b.include(u.PhysicallyBasedRendering,a),b.fragment.include(g.MixExternalColor),k.add(c.glsl`
      void main() {
        discardBySlice(vpos);
        ${a.hasColorTexture?c.glsl`
        vec4 texColor = texture2D(tex, vuv0);
    ...
    `));b.include(C.DefaultMaterialAuxiliaryPasses,a);return b}var G=Object.freeze({__proto__:null,build:h});e.RealisticTreeShader=G;e.build=h});