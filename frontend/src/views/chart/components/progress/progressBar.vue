        ...
        if(chart.data.series) {
          ...
          for(let i=0;i<data.length;i++) {
            if(data[i].dimensionList[0].value.includes('满意度')) {
              this.progressData.totality.name = data[i].dimensionList[0].value
              this.progressData.totality.value = data[i].value
            } else {
             let obj = {
              name: data[i].dimensionList[0].value,
              value: data[i].value
             }
             arr.push(obj)
            }
          }
          this.progressData.otherInfo = arr
