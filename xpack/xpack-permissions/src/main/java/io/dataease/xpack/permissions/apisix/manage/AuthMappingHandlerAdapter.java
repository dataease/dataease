...
public class AuthMappingHandlerAdapter extends RequestMappingHandlerAdapter {
    ...
    private int getBodyAdviceCount(Class<?> adviceType) {
        ...
        return RequestBodyAdvice.class.isAssignableFrom(adviceType) ?
                AuthMappingHandlerAdapter.getAdviceByType(advice, RequestBodyAdvice.class).size() :
                AuthMappingHandlerAdapter.getAdviceByType(advice, ResponseBodyAdvice.class).size();
    }
    ...
    public Object[] getParams(HttpServletRequest request,
        ...
        AuthServletInvocableHandlerMethod invocableMethod = createInvocableHandlerMethod(handlerMethod);
        if (this.argumentResolvers != null) {
            invocableMethod.setHandlerMethodArgumentResolvers(this.argumentResolvers);
        }
        if (this.returnValueHandlers != null) {
        ...
        modelFactory.initModel(webRequest, mavContainer, invocableMethod);
        mavContainer.setIgnoreDefaultModelOnRedirect(this.ignoreDefaultModelOnRedirect);
        Object[] args = invocableMethod.getMethodArgumentValues(webRequest, mavContainer);
        return args;
    }
